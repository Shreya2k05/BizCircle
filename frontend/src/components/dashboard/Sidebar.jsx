...
const Sidebar = () => {
  ...
  const menuItems = [
    ...
    { name: "Referral Center", icon: GitMerge, href: "/dashboard/referrals" },
  ...
  const activeClass = "bg-blue-600 text-white font-bold shadow-lg shadow-blue-100 ring-1 ring-blue-600";
  const inactiveClass = "text-slate-500 hover:text-blue-600 hover:bg-blue-50/50 transition-all duration-300";
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`
        fixed inset-y-0 left-0 z-40 w-64 glass border-r border-slate-200/50
        ...
        <div className="flex flex-col h-full p-6">
          ...
          <div className="flex items-center justify-between gap-3 mb-10 px-2">
            <div className="flex items-center gap-3">
              ...
              <span className="font-display font-bold text-xl text-slate-900 tracking-tight">BizCircle</span>
            </div>
          </div>
          ...
          <div className="mb-8 px-2">
            <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-100 rounded-2xl p-4 shadow-sm shadow-amber-900/5 relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-16 h-16 bg-amber-200 opacity-20 rounded-full blur-2xl -translate-y-1/2 translate-x-1/2 group-hover:scale-150 transition-transform duration-700" />
              <div className="flex items-center gap-3 relative z-10">
                <div className="w-8 h-8 rounded-lg bg-amber-400 flex items-center justify-center shadow-md shadow-amber-900/20 animate-float-subtle">
                  <Star className="text-slate-900 fill-slate-900 w-4 h-4" />
                </div>
                <div>
                  <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest">BizPoints Balance</p>
                  <p className="text-lg font-black text-slate-900 leading-none">{points}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="pt-6 border-t border-slate-50 mt-auto">
            ...
            {user?.role === "admin" && (
              <Link
                href="/admin"
                onClick={() => setIsOpen(false)}
                className="flex items-center gap-3 w-full px-4 py-3 rounded-xl text-sm text-indigo-600 hover:bg-indigo-50 transition-colors duration-200 font-semibold mb-1"
              >
                <ShieldCheck size={18} />
                Admin Panel
              </Link>
