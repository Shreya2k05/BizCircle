...
const CircleCard = ({ _id, name, domain, location, members, isJoined: propIsJoined, icon, color, description, memberCount, type, createdBy, membershipRole }) => {
  ...
  return (
    <Link
      ...
      className="premium-card p-6 flex flex-col h-full group relative overflow-hidden"
    >
      ...
      <div className="flex justify-between items-start mb-6">
        <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${color || 'from-blue-600 to-indigo-600'} text-white flex items-center justify-center text-2xl font-black shadow-lg shadow-blue-100 animate-float-subtle`}>
          {icon || name.charAt(0)}
        </div>
        <div className="flex flex-col items-end gap-2">
          <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-3 py-1.5 rounded-full uppercase tracking-widest border border-blue-100 shadow-sm">
            {domain}
          </span>
          {canDelete && (
            <button
      ...
      <h3 className="text-xl font-black text-slate-800 mb-2 truncate font-display">{name}</h3>
  ...
  return (
    <section id={id} className="mb-10 pt-4 -mt-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-6">
        <h2 className="text-2xl font-black text-slate-900 font-display tracking-tight">{title}</h2>
        {showLimitMessage && (
