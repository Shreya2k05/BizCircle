...
import { Bell, Send, History, CheckCircle, Clock, Smartphone, Mail, Globe, Loader2 } from 'lucide-react';
...
export default function NotificationsSection() {
  ...
  const handleSend = async () => {
    ...
    try {
      setLoading(true);
      const res = await api.post("/api/notifications/send-global", {
        title,
        message,
        target
      });
        ...
        <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
          ...
          <div className="space-y-6">
            ...
            <button 
              onClick={handleSend}
              disabled={loading}
              className="w-full py-4 bg-blue-600 rounded-2xl text-sm font-extrabold text-white hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-blue-100 flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
