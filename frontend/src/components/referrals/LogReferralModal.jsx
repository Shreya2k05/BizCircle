...
export default function LogReferralModal({ isOpen, onClose, onSuccess }) {
  ...
  // Helper: deduplicate an array of circles/groups by _id
  const dedupeById = (arr) => [
    ...new Map(arr.filter(Boolean).map(c => [c._id, c])).values()
  ];

  useEffect(() => {
    if (isOpen) {
      // Default: show current user's own circles when no receiver selected
      const defaultCircles = dedupeById([
        ...(user?.circles || []),
        ...(user?.joinedGroups || []),
      ]);
