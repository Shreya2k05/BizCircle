...
export default function ReferralList({ referrals, type, onUpdate }) {
  ...
  const handleManualVerify = async (id) => {
    setActionLoading(id);
    try {
      if (confirm("Demo Mode: Force verify this referral without email?")) {
        await api.put(`/api/referrals/${id}/manual-verify`);
        await fetchProfile();
        onUpdate();
        alert("Referral verified manually (Demo Mode)!");
      }
    } catch (err) {
      console.error("Failed to manual verify:", err);
      alert(err.response?.data?.message || "Failed to manual verify");
    } finally {
      setActionLoading(null);
    }
  };
               ...
               <div className="flex gap-2">
                 ...
                 {type === 'shared' && ref.status === 'Pending' && (
                    <div className="flex gap-4">
                      ...
                      <button 
                        disabled={actionLoading === ref._id}
                        onClick={() => handleManualVerify(ref._id)}
                        className="flex items-center gap-1.5 text-amber-500 hover:text-amber-600 text-[10px] font-bold uppercase transition-colors border border-amber-100 px-2 py-1 rounded-md bg-amber-50/50"
                        title="Viva Backup: Force verify if email is delayed"
                      >
                        {actionLoading === ref._id ? <Loader2 size={10} className="animate-spin" /> : <ShieldCheck size={10} />}
                        Force Verify
                      </button>
                    </div>
                  )}
                     ...
                     {ref.status === 'Verified' && (
                       <div className="flex gap-2">
                         ...
                         <button 
                           ...
                           className="flex items-center gap-1.5 bg-white border border-red-200 text-red-500 hover:bg-red-50 px-3 py-1.5 rounded-lg text-xs font-bold transition-all shadow-sm"
                         >
                           Reject
                         </button>
                       </div>
                     ...
                     {ref.status === 'Pending' && (
                       <div className="flex items-center gap-3">
                         ...
                         <button 
                           disabled={actionLoading === ref._id}
                           onClick={() => handleStatusUpdate(ref._id, 'Rejected')}
                           className="text-red-400 hover:text-red-600 text-[10px] font-bold uppercase underline-offset-2 hover:underline"
                         >
                           Decline
                         </button>
                       </div>
