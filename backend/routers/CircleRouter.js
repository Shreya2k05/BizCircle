...
router.get('/', async (req, res) => {
    try {
        ...
        // 1. Optional Authentication Check to find user associations
        ...
        if (authHeader) {
            try {
                ...
                const decoded = jwt.verify(authHeader.split(' ')[1], process.env.JWT_SECRET || 'mytopseret');
                userId = decoded.id;

                const memberships = await CircleMember.find({ user: userId });
                excludedIds = memberships.map(m => m.circle.toString());
            } catch (err) {
        ...
        // 3. Apply Filtering (Hide owned/joined circles)
        if (userId) {
        ...
        const circles = await Circle.find(query)
            .select('name domain location memberCount description relatedDomains color icon isPrivate status createdBy')
...
router.post('/join', auth, async (req, res) => {
    try {
        ...
        if (status === 'Approved') {
            await Promise.all([
                User.findByIdAndUpdate(userId, { $push: { circles: circleId }, $inc: { points: 10 } }),
...
router.put('/:id', auth, async (req, res) => {
    try {
        ...
        if (!adminCheck) {
            return res.status(403).json({ message: 'Only circle admins can update circle settings' });
        }
...
router.put('/:id/members/:memberId', auth, async (req, res) => {
    try {
        ...
        if (status === 'Approved' && member?.user) {
            await Promise.all([
                User.findByIdAndUpdate(member.user._id, { $push: { circles: circle._id }, $inc: { points: 10 } }),
