...
const sendVerificationEmail = async ({ to, candidateName, senderName, verificationLink }) => {
  ...
  try {
    ...
    console.error('❌ SMTP Error (sendVerificationEmail):', err.message);
  }
};
...
const sendReferralReceivedEmail = async ({ to, receiverName, senderName, candidateName }) => {
  ...
  try {
    ...
    console.log(`📧 Alert email sent to receiver: ${to}`);
  } catch (err) {
    console.error('❌ SMTP Error (sendReferralReceivedEmail):', err.message);
  }
};
